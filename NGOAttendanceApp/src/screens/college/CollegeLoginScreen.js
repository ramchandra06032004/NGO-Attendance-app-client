...
export default function CollegeLoginScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: (colors.backgroundColors && colors.backgroundColors[0]) || '#ecfeff' }]}>
      <View style={[styles.card, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
        <Text style={[styles.cardTitle, { color: colors.header }]}>College Login</Text>

        <Text style={[styles.label, { color: colors.textPrimary }]}>Select college</Text>
        <ScrollView style={[styles.picker, { backgroundColor: colors.iconBg, borderColor: colors.border }]} contentContainerStyle={{ padding: 6 }}>
          {colleges.map(c => {
            ...
            return (
              <Pressable
                ...
                style={[
                  styles.pickerItem,
                  active && {
                    backgroundColor: active ? (darkMode ? '#ffffff12' : '#fcfbf7ff') : undefined,
                    borderWidth: active ? 1 : 0,
                    borderColor: active ? colors.accent : colors.border,
                  },
                ]}
              >
                <Text style={[styles.pickerItemText, active && styles.pickerItemTextActive, { color: colors.textPrimary }]}>{c}</Text>
        ...
        {selectedCollege ? (
          <View style={styles.loginArea}>
            <Text style={[styles.logging, { color: colors.textPrimary }]}>Logging in as <Text style={{ fontWeight: '700', color: colors.header }}>{selectedCollege}</Text></Text>
            <TextInput placeholder="Email" value={email} onChangeText={setEmail} style={[styles.input, { backgroundColor: colors.iconBg, borderColor: colors.border, color: colors.textPrimary }]} keyboardType="email-address" autoCapitalize="none" placeholderTextColor={colors.textSecondary} />
          ...
          <Text style={[styles.hint, { color: colors.textSecondary }]}>Please select a college to continue</Text>
        )}

      </View>

      <TouchableOpacity onPress={goBack} style={styles.cancel}><Text style={{ color: colors.textPrimary }}>Cancel</Text></TouchableOpacity>
    </View>
  );
}
