        ...
        renderItem={({ item }) => (
          <View style={[styles.row, { backgroundColor: colors.cardBg }]}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.meta, { color: colors.textSecondary }]}>{item.college} • {item.class}</Text>
            </View>

            <View style={styles.actions}>
              <TouchableOpacity style={[styles.markBtn, item.status === 'present' && styles.activePresent, { backgroundColor: item.status === 'present' ? '#bbf7d0' : colors.cardBg }]} onPress={() => markAttendance(event.id, item.id, 'present')}>
                <Text style={[styles.markText, { color: colors.textPrimary }]}>Present</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.markBtn, item.status === 'absent' && styles.activeAbsent, { backgroundColor: item.status === 'absent' ? '#fecaca' : colors.cardBg }]} onPress={() => markAttendance(event.id, item.id, 'absent')}>
                <Text style={[styles.markText, { color: colors.textPrimary }]}>Absent</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
